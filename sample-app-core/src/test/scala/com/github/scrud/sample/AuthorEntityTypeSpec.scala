...
class AuthorEntityTypeSpec extends FunSpec with MustMatchers {
  ...
  it("must calculate the book count") {
    ...
    val authorId = commandContext.save(Author, MapStorage, None, new MapStorage(authorType.nameField -> Some("Tolkien")))
    commandContext.save(Book, MapStorage, None, new MapStorage(
    ...
    val author = commandContext.findAll(Author, MapStorage).head
    author.get(authorType.bookCount) must be (Some(4))
  }

  it("must calculate the author's nick name using the name and bookCount") {
    val commandContext = new CommandContextForTesting(entityTypeMap)
    val authorId = commandContext.save(Author, MapStorage, None, new MapStorage(
